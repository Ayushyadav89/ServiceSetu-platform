    ...
    timer.current = setTimeout(async () => {
      try {
        const { data } = await api.get(
          `/workers/suggestions?q=${encodeURIComponent(val)}&type=${type}`
        )
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs">
            Enter your area name and city to find nearby workers
          </p>
        </div>
      </div>
          ...
          {result.count === 0 && (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Try a different area, city, or remove the skill filter
              </p>
            </div>
          )}
        </>
...
function WorkerRow({ worker, onBook }) {
  ...
  // Calculate final price customer would pay
  ...
  return (
    <div className="flex items-start gap-3 bg-slate-50 hover:bg-slate-100 rounded-xl px-4 py-3 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1 mt-1">
          {worker.skills?.map((s) => (
            <span
              key={s}
              className="text-xs bg-white border border-slate-200 text-slate-600 px-2 py-0.5 rounded-full capitalize"
            >
              {s.replace('_', ' ')}
            </span>
          ))}
        ...
        {/* Price row — only shown if worker has set a price */}
        {workerPrice > 0 && (
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-200">
            <span className="text-xs text-slate-500">
              Worker asks{' '}
              <span className="font-semibold text-slate-700">₹{workerPrice}</span>
            </span>
      ...
      {/* Book button — only shown for available workers */}
